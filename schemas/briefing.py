...
class ConceptExplanation(BaseModel):
    ...
    plain_english: str = Field(
        description="2-3 sentence plain-English explanation for a software engineer"
    )
    example: str = Field(
        description="A concrete, intuitive example or analogy"
    )
    why_it_matters: str = Field(
        description="Why this concept is important in practice"
    )
    connected_to_today: str = Field(
        description="How this concept relates to today's papers or research"
    )
...
class DailyBriefing(BaseModel):
    ...
    emerging_themes: str = Field(
        description="2-3 paragraph analysis of emerging trends across all sources"
    )
